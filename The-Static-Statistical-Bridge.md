...
> **Note:** This Fix Log is retained for version-history transparency during the review process. It will be removed before formal submission; the final manuscript will contain only the paper content.

**Date**: 2026-07-01  
**Based on**: EAS-v3-Review-Report.md  

| ID | Issue | Fix Applied |
|----|-------|-------------|
| C1 | `csprng_high_epiplexity` theorem name contradicts its statement (name says "high epiplexity" but CSPRNG has low $S_T$) | Renamed to `csprng_high_entropy`; added Remark 2.2a documenting Apoth3osis naming error and author discrepancy |
| C2 | Apoth3osis formalization lists "Jonathan Finzi" as author; original paper first author is Marc Finzi (CMU). Also lists Olah and Nanda who are not co-authors | Added Remark 2.2a explicitly marking Apoth3osis as independent third-party formalization, not original-author verification; noted all author discrepancies |
| C3 | GitHub URL `github.com/simulai/EAS-Paper` resolves to unrelated IBM project | Updated all references: noted URL could not be independently verified, directed readers to Zenodo DOI 10.5281/zenodo.21038854 as the verifiable archive |
| C4 | $\Delta_{\mathrm{waste}}$ naming creates counterintuitive conclusions ("ideal = maximum waste, degenerate = zero waste") | Renamed to $\Delta_{\mathrm{index}}$ ("Representation Indexing Cost"); updated all occurrences (~30 instances); rewrote §5.2.2 explanation; fixed misleading statements in §6.2 and §7.1 about "reducing waste" |
| L3/C5 | Example 5.6: $H(Y)$ listed as 1.562 but correct value is 1.75; $\Delta_{\mathrm{dist}}$ listed as 0.438 but correct value is 0.25 | Corrected all numerical values; added explicit $p_Y$ distribution for verifiability; verified with code |
| M1 | Theorem C decomposition presented as "remarkable" new discovery; actually algebraic reorganization of standard identities | Removed "remarkable"; added remark after Theorem C acknowledging algebraic nature; softened §6.1 language; noted $\Delta_{\mathrm{index}} + \Delta_{\mathrm{dist}} = \log_2 n$ is tautological |
| M3 | §4.4 lists six classes of non-fullness counterexamples but only one (within-fiber randomization) is formally proved | Retitled §4.4 as "Speculative Classification"; added explicit note that only within-fiber randomization is formally proved; marked remaining five as plausible conjectures |
| M5 | Corollary 3.2 uses $H(E)$ (syntactically incorrect for a set) and lacks $\epsilon < \log_2(m/n)$ restriction | Corrected $H(E) \to H(X)$; added restriction $0 < \epsilon < \log_2(m/n)$ for non-triviality |
| M7 | OEV mapping presented as "structural decomposition" without formal argument | Downgraded to "heuristic correspondence"; added caveats about lack of formal proof and weakest link ($\Delta_{\mathrm{index}} \to$ Executor); softened §6.2.3 from "structurally necessary" to "heuristic" |
| L1 | $S_T$ vs $K_T$ notation difference between this paper and Finzi et al. original not explained | Added notation note in §1.5 explaining that $S_T$ follows Apoth3osis formalization while Finzi et al. use $K_T$ |
| L2 | §7.4 assumes additivity of epiplexity across streams without discussing when independence fails | Added note about subadditivity/superadditivity and the strong idealization of independence |
| L4 | "Discrete–Continuous Duality" is non-standard terminology that could mislead | Added parenthetical note at first substantive use explaining it is a descriptive term, not classical mathematical duality |
| L5 | Three "Zhang (2026)" references in bibliography not distinguished | Added a/b/c suffixes; updated in-text citations accordingly; replaced `[Duality-Proof]` with `[Zhang, 2026c]` |
| L6 | Proposition 5.3 convergence may be extremely slow but this is not stated | Added "at possibly non-uniform rate" to proposition statement |
| L7 | §6.2 "Randomness Is a Superpower" is over-rhetorical; non-fullness ≠ practical superiority | Renamed to "Stochastic Methods Provide Additional Representational Capacity"; added caveat that categorical difference ≠ practical guarantee |
...
**Date**: 2026-07-02
**Based on**: EAS-v4-Second-Review-Report.md

| ID | Priority | Issue | Fix Applied |
|----|----------|-------|-------------|
| N1 | P0 | §7.1 still uses "Wasted representational capacity" for $\Delta_{\mathrm{index}}$, contradicting §5.2.2 | Rewrote §7.1 $\Delta_{\mathrm{index}}$ description to align with §5.2.2 "indexing cost" semantics; added cross-reference to §5.2.2 |
| M4 | P1 | Uniform distribution assumption not noted in Abstract/Introduction | Added "Important limitation" paragraph before Keywords in Abstract; references §8.2 Limitation 3 |
| M2 | P1 | $P^*_f$ limitation discussed only in §8.2, not in §4.1 where $\Phi$ is defined | Added Remark 4.1a in §4.1 explaining that $P^*_f$ non-optimality does not affect faithfulness; cross-references §8.2 Limitation 5 |
| N2 | P2 | §7.4 "Zhang (2026)" missing a/b/c suffix | Corrected to "Zhang (2026a)" |
| N3 | P3 | Fix Log retained in paper body, unsuitable for formal submission | Added note that Fix Log will be removed before formal submission |
...
**Date**: 2026-07-01
**Change type**: Narrative/thematic enrichment (no mathematical content modified)

| ID | Type | Change |
|----|------|--------|
| N1 | Narrative | Added §1.4 ("The Discrete-Continuous Relationship: Beyond Bridging") in Introduction, reframing the three theorems as epistemological contribution, not merely technical; critiquing three traditional views of discrete-continuous relationship |
| N2 | Narrative | Added §6.4 ("The Discrete-Continuous Relationship: Beyond Bridging") in Discussion, providing unified interpretive reading of Theorems A/B/C; arguing non-fullness is a feature; discussing implications for scaling laws, intrinsic stochasticity, and the discrete-continuous debate |
| N3 | Narrative | Added one sentence to §9 Conclusion: discrete-continuous as observation modes, not two worlds needing a bridge |
| N4 | Editorial | Renumbered §1.4→§1.5, §1.5→§1.6, §6.4→§6.5 to accommodate new sections |

**No new theorems, proofs, or mathematical content were added. Existing mathematical results are unchanged.**
